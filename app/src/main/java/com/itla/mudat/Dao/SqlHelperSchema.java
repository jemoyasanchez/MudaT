...
public class SqlHelperSchema {
     ...
     public static final String ANUNCIO_TABLE="CREATE TABLE IF NOT EXISTS "+Anuncio.nomtableanuncio+" ("  +Anuncio.nomid+"   INTEGER PRIMARY KEY AUTOINCREMENT,"+Anuncio.nomcategoria+" INTEGER NOT NULL,"+Anuncio.nomusuario+" INTEGER NOT NULL, "+Anuncio.nomfecha+" REAL NOT NULL,"+Anuncio.nomcondicion+" INTEGER,"+Anuncio.nomprecio+"  NUMERIC DEFAULT 0,"+Anuncio.nomtitulo+" TEXT NOT NULL,"+Anuncio.nomtitulo+" TEXT NOT NULL,"+Anuncio.nomdetalle+" TEXT, FOREIGN KEY("+Anuncio.nomusuario+") REFERENCES "+Usuario.nomtableUsuario+"("+Usuario.nomid+"), FOREIGN KEY("+Anuncio.nomcategoria+") REFERENCES "+Categoria.nomtableCategoria+"("+Categoria.nomid+"));";
